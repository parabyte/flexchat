...
static session *
find_context_by_name (const char *servname, const char *channel, server *current_server)
{
...
cmd_doat (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	for (i = 0; channels[i] && *channels[i]; ++i)
	{
		...
		if ((ctx = find_context_by_name (serv, chan, sess->server)))
...
cmd_load (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	PrintText (sess, _("Plugin support has been removed; only loading command files with -e is available.\n"));
	return TRUE;
}
...
const struct commands xc_cmds[] = {
	...
	{"LOAD", cmd_load, 0, 0, 1, N_("LOAD -e <file>, loads commands from a file")},
